...
public class HammerStatsScript : MonoBehaviour {
	...
	public void Advance () {
		if (t > 0.75)
			UnityEngine.SceneManagement.SceneManager.LoadScene ("Selling");
	}

	void Start () {
		ItemSword finalSword = (ItemSword) GameController.control.GetItem ("selling/sword");
		dmgThings = ((float) finalSword.GetDamage () / GameController.control.GetInt ("hammer/dmg")) * 100 - 100;
		rngThings = ((float) finalSword.GetRange () / GameController.control.GetInt ("hammer/rng")) * 100 - 100;
		dwrThings = ((float) finalSword.GetDurability () / GameController.control.GetInt ("hammer/dwr")) * 100 - 100;
